...
class HBNBCommand(cmd.Cmd):
    """Defines the Holberton school BnB cmd interpreter.
    ...
    def default(self, arg):
        """Default state for cmd module when given invalid input."""
        argdict = {
            "all": self.do_all,
            "show": self.do_show,
    ...
    def do_EOF(self, arg):
        """EOF to signal progarm exit."""
        print("")
        return True

    def do_create(self, arg):
        """Usage: create <class>
        To create a new class instance that prints its id.
        """
        argl = parse(arg)
        if len(argl) == 0:
    ...
    def do_update(self, arg):
       ...
       <class>.update(<id>, <dictionary>)
        ...
        given attributes key/value pair or dictionary."""
        argl = parse(arg)
        objdict = storage.all()
